...
test('restrictedMode is the single export and reading policy boundary', () => {
  ...
  assert.match(server, /app\.get\('\/search-index\.json'/)
  assert.match(server, /const SEARCH_INDEX_VERSION = 1/)
...
test('restricted mode hides export and Mermaid externalization affordances', () => {
  assert.match(header, /if \(!docsRestrictedMode\)/)
  assert.match(header, /doc-search-trigger/)
  ...
  assert.match(header, /doc-search\.js/)
